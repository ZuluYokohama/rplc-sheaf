...
Sparse ALU
----------
  import sparse_backend; sparse_backend.install(sys.modules[__name__])
...
def sheaf_lambda1(X, k_nn=4, n_cycles=0, twist=0.0, backend="auto"):
    """Obstruction λ1 and h0. backend: auto|dense|sparse.
    Dense is default path here. For sparse at scale:
      import sparse_backend; sparse_backend.install(sys.modules[__name__])
    """
    if len(X) < 6:
        return 0.0, 1
...
def audit_stratum(X, mask, n_ctrl=5, seed=0, use_persistence=False, audit_margin=0.1):
    Xs = X[mask]
    if use_persistence:
    ...
    survives = bool(lam < np.mean(rands) - audit_margin * max(np.std(rands), 1e-6))
...
def isa_exec(X, program=None, seed=0, percentile=75, include_remainder=False):
    if program is None:
    ...
    def do_core_step(step):
        ...
        if core is None:
            trace.append({"op": "HALT", "reason": "no_core"}); halted = True; return False
        if len(living) < 28: return False
        Xl = X[living]
        C = cdist(Xl, core)
        aff = np.exp(-C / (np.median(C) + 1e-9)).max(1)
        mask = aff >= np.percentile(aff, percentile)
        n = int(mask.sum())
        if n < 12:
            trace.append({"op": "HALT", "reason": "stratum_small"}); halted = True; return False
    ...
    def do_rotate(step):
        ...
        if len(intervals) < 25: return False
    ...
    for instr in program:
        ...
        if op == "CORE":
            ...
            for s in range(1, int(instr.get("max", 3)) + 1):
                if not do_core_step(s): break
            if halted: break
        elif op == "ROTATE":
            trace.append({"op": "ROTATE"}); do_rotate(99)
        elif op == "HALT":
            trace.append({"op": "HALT", "living": len(living)}); break
        else:
            trace.append({"op": "HALT", "reason": "unknown_op", "raw": str(op)}); halted = True; break
    cert = {"n0": int(X.shape[0]), "dims": int(X.shape[1]),
            "opened_steps": int(sum(1 for e in ladder if e.get("survives"))),
            "remainder_n": len(living), "ladder": ladder, "trace": trace,
            "program": program, "version": __version__}
    if include_remainder: cert["remainder"] = living
    return cert
...
def isa_run(X, max_steps=4, percentile=75, use_persistence=False, seed=0, enable_rotate=True):
    ...
    if enable_rotate: program.append({"op": "ROTATE"})
...
def run(X, program=None, seed=0, verify=True, include_remainder=False):
    if program is None:
        program = [{"op": "CORE"}, {"op": "STEP", "max": 3}, {"op": "ROTATE"}, {"op": "HALT"}]
    cert = isa_exec(X, program=program, seed=seed, include_remainder=include_remainder)
    report = verify_certificate(cert, X=X, seed=seed) if verify else None
    return cert, report

def run_domain(domain="frb", n=160, seed=0, **kwargs):
    if domain == "frb": X, rows = synthetic_frb(n, seed=seed)
    elif domain == "materials": X, rows = synthetic_materials(n, seed=seed)
    else: raise ValueError("domain must be 'frb' or 'materials'")
...
def _cli():
    ...
    p = argparse.ArgumentParser(description="RPL-C / RPL-ISA")
    p.add_argument("--domain", choices=["frb", "materials", "csv"], default="frb")
    p.add_argument("--csv", default=None); p.add_argument("--features", default=None)
    p.add_argument("--n", type=int, default=160); p.add_argument("--steps", type=int, default=3)
    p.add_argument("--seed", type=int, default=0); p.add_argument("--no-rotate", action="store_true")
    p.add_argument("--no-verify", action="store_true"); p.add_argument("--out", default=None)
    args = p.parse_args()
    program = [{"op": "CORE"}, {"op": "STEP", "max": args.steps}]
    if not args.no_rotate: program.append({"op": "ROTATE"})
    program.append({"op": "HALT"})
    if args.domain == "csv":
        if not args.csv or not args.features:
            print("csv requires --csv and --features", file=sys.stderr); sys.exit(1)
        X, _, _ = load_csv_features(args.csv, [c.strip() for c in args.features.split(",")])
    ...
    out = {"certificate": cert, "verify": report}
    print(json.dumps(out, indent=2, default=str))
    if args.out:
        with open(args.out, "w") as f: json.dump(out, f, indent=2, default=str)
